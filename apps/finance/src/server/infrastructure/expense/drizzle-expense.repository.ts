...
import { ExpenseAllocation } from "@/server/domain/expense/expense-allocation";
  ...
  async save(expense: Expense): Promise<void> {
    ...
    this.uow.register(async (tx) => {
      ...
      if (d.allocations.length > 0) {
        await tx.insert(expenseAllocationsV2).values(
          d.allocations.map((a) => ({
            id: a.data.id.value,
  ...
  async findOne(id: Id, userId: Id): Promise<Expense | null> {
    const [txRow] = await db
      ...
      .where(
        and(
          ...
          eq(transactionsV2.type, "expense"),
    ...
    const [allocationRows, tagRows] = await Promise.all([
      db
        ...
        .where(eq(expenseAllocationsV2.transactionId, id.value)),
      db
        .select({
          id: tagsV2.id,
          name: tagsV2.name,
